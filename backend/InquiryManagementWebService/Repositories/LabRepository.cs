...
ORDER BY g.RegistrationNo;
        ...
        public async Task<IEnumerable<SampleDetails>> GetSampleDetailsByIdAsync(string regNo)
        {
            ...
            var query = @"
                ...
                WHERE 
                    t1.TRN1PLANTCD = 'P001' 
                    AND t1.TRN1DATE BETWEEN '2025-04-01 00:00:00.000' AND '2030-03-31 00:00:00.000'
        ...
        public async Task<IEnumerable<SampleDetails>> GetPendingParametersFromQAAsync(ParameterRequest request)
        {
...
(
    SELECT
        t1.TRN1REFNO AS RegistrationNo,
        t1.TRN1DATE AS RegistrationDate,
        t2.TRN2REFNO AS TRN2REFNO,
        t2.TRN2HEADER AS ParaCode,
        t2.TRN2PRODALIAS AS SampleName,
        t2.Trn2Pardate AS TatDate,
        t2.TRN2COMPLETIONDT AS AnalysisCompletionDateTime,
        t2.TRN2MdateofReport AS MailingDate,
        t2.TRN2REPODT AS ReportIssueDate,
        t2.TRN2DEPARTCD AS DeptCode,
        L.CODEDESC AS Lab,
        p.headdesc AS Parameter,
        r.RegisVal,
        s.SampleCnt,
        ROW_NUMBER() OVER (PARTITION BY t2.TRN2REFNO, t2.TRN2HEADER ORDER BY t2.TRN2REFNO) AS RN
    FROM trn105 t1
    INNER JOIN trn205 t2 ON t1.TRN1REFNO = t2.TRN2REFNO
    INNER JOIN OHEADMST p ON t2.TRN2HEADER = p.headcd
    INNER JOIN OCODEMST L ON t2.TRN2DEPARTCD = L.CODECD AND L.CODETYPE = 'DM'
    ...
    WHERE 
        t1.TRN1PLANTCD = 'P001'
        AND t1.TRN1DATE BETWEEN '2025-04-01' AND '2030-03-31'
        AND t2.TRN2COMPLETIONDT IS NOT NULL
        AND t2.TRN2MdateofReport IS NULL
        AND t2.TRN2COMPLETIONDT BETWEEN @FromDate AND @ToDate
        AND (@Labs IS NULL OR L.CODEDESC IN (SELECT Value FROM dbo.SplitStrings(@Labs, ',')))
)
SELECT
    RegistrationNo,
    CONVERT(NVARCHAR(10), RegistrationDate, 103) AS RegistrationDate,
    CONVERT(NVARCHAR(10), ReportIssueDate, 103) AS ReportIssueDate,
    SampleName,
   TatDate,
    CASE
        WHEN AnalysisCompletionDateTime IS NOT NULL THEN FORMAT(AnalysisCompletionDateTime, 'dd/MM/yyyy HH:mm:ss')
        ELSE ''
    END AS AnalysisCompletionDateTime,
    CONVERT(NVARCHAR(10), MailingDate, 103) AS MailingDate,
    ParaCode,
    Parameter,
    Lab,
    CAST(RegisVal / NULLIF(SampleCnt,0) AS DECIMAL(18,2)) AS DistributedRegisVal
FROM UniqueParams
WHERE RN = 1 
ORDER BY RegistrationNo, Parameter;
            ";
        ...
        public async Task<IEnumerable<LabParameterOverview>> GetPendingParametersOverviewFromQAAsync(ParameterRequest request)
        {
...
(
    SELECT 
        t1.TRN1REFNO       AS RegistrationNo,
        L.CODEDESC         AS LabName,
        t2.TRN2HEADER      AS ParameterCode
    FROM trn105 t1
    ...
    WHERE 
        ...
        AND (
                @Labs IS NULL 
                OR L.CODEDESC IN (SELECT value FROM dbo.SplitStrings(@Labs, ','))
            )
),
UniqueParams AS
(
...
(
    SELECT 
        RegistrationNo,
        LabName,
        COUNT(ParameterCode) AS UniqueParameterCount
    FROM UniqueParams
    GROUP BY 
        RegistrationNo,
        LabName
...
SELECT 
    LabName,
    SUM(UniqueParameterCount) AS ParameterCount
FROM RegLevelCount
GROUP BY LabName
ORDER BY LabName;
            ";
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                return await connection.QueryAsync<LabParameterOverview>(query, new
                {
                    FromDate = request.FromDate,
                    ToDate = request.ToDate,
                    Labs = labs,
